...
import { OrderType, getOrderCount, createOrder, handleOrder, DecreasePositionSwapType } from "../../utils/order";
...
describe("Guardian.MarketDecrease", () => {
  ...
  it("MarketDecrease with a price feed token", async () => {
    const initialUSDCBalance = expandDecimals(50_000, 6); // $50k USDC
    expect(await getOrderCount(dataStore)).eq(0);
    ...
    await handleOrder(fixture, {
      ...
      execute: {
        tokens: [wnt.address],
        minPrices: [expandDecimals(5500, 4)], // ETH goes up 10%
        maxPrices: [expandDecimals(5500, 4)],
        precisions: [8, 18],
        gasUsageLabel: "orderHandler.executeOrder",
    ...
    // Position size 40 ETH -> $500 profit per ETH
    // $20,000 profit / $5,500 = ~3.636363636 ETH
    // Collateral 50,000 USDC

    const profitAmount = ethers.utils.parseEther("3.636363636363636363");
    const collateralAmount = expandDecimals(50_000, 6);

    expect((await usdc.balanceOf(user1.address)).sub(userUSDCBalBefore)).to.eq(collateralAmount); // Swapped to USDC
    expect((await wnt.balanceOf(user1.address)).sub(userWNTBalBefore)).to.eq(profitAmount);
  });

  it("MarketDecrease with several price feed tokens in swapPath", async () => {
    const initialUSDCBalance = expandDecimals(50_000, 6); // $50k USDC
    expect(await getOrderCount(dataStore)).eq(0);
    ...
    await handleOrder(fixture, {
      ...
      execute: {
        ...
        precisions: [8, 18],
        gasUsageLabel: "orderHandler.executeOrder",
        priceFeedTokens: [usdc.address, usdt.address],
      },
    });
  ...
  it("MarketDecrease to realize losses on a short position -- collateralToken != pnlToken", async () => {
    ...
    const lossAmount = ethers.utils.parseEther("3.636363636363636363");
