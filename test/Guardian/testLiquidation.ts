...
describe("Guardian.Liquidation", () => {
  ...
  it("Liquidate someone in profit & they receive both pnl and collateral tokens as output", async () => {
    ...
    // $450,000 Position => >100x leverage => liquidated
    // pnlAmount that pool pays => $300 / $4000 = 0.075 ETH
    // Collateral amount transferred to the user => 5,000 USDC
    const pnlAmount = ethers.utils.parseEther("0.075");

    expect(poolBalBefore.sub(await getPoolAmount(dataStore, solUsdMarket.marketToken, wnt.address))).to.eq(pnlAmount);
    expect((await wnt.balanceOf(user1.address)).sub(userWNTBalBefore)).to.eq(0);
    expect((await ethers.provider.getBalance(user1.address)).sub(userNativeBalBefore)).to.eq(pnlAmount);
    expect((await usdc.balanceOf(user1.address)).sub(userUSDCBalBefore)).to.eq(initialUSDCBalance);
  });

  it("Use a price feed token to do a liquidation", async () => {
    const initialUSDCBalance = expandDecimals(5_000, 6); // 5,000 USDC
    expect(await getOrderCount(dataStore)).eq(0);
    ...
    const userNativeBalBefore = await ethers.provider.getBalance(user1.address);
    const userUSDCBalBefore = await usdc.balanceOf(user1.address);
    const poolBalBefore = await getPoolAmount(dataStore, solUsdMarket.marketToken, usdc.address);
    ...
    await executeLiquidation(fixture, {
      ...
      collateralToken: usdc,
      isLong: true,
      tokens: [wnt.address, solAddr],
      tokenOracleTypes: [TOKEN_ORACLE_TYPES.DEFAULT, TOKEN_ORACLE_TYPES.DEFAULT],
      precisions: [8, 8],
      priceFeedTokens: [usdc.address],
      minPrices: [expandDecimals(5000, 4), expandDecimals(14, 4)], // SOL goes to $14
      maxPrices: [expandDecimals(5000, 4), expandDecimals(14, 4)],
      gasUsageLabel: "liquidationHandler.executeLiquidation",
    });

    // Pool gets entire collateral, trader receives nothing

    expect((await getPoolAmount(dataStore, solUsdMarket.marketToken, usdc.address)).sub(poolBalBefore)).to.eq(
      initialUSDCBalance
    );
    expect((await wnt.balanceOf(user1.address)).sub(userWNTBalBefore)).to.eq(0);
    expect((await ethers.provider.getBalance(user1.address)).sub(userNativeBalBefore)).to.eq(0);
