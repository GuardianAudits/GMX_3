...
describe("Guardian.MktSwapOrder", () => {
  let fixture;
  let user0, user1;
  let dataStore, ethUsdMarket, ethUsdSpotOnlyMarket, solUsdMarket, wnt, usdc, wbtc, baseOrderUtils;

  beforeEach(async () => {
    fixture = await deployFixture();
    ({ user0, user1 } = fixture.accounts);
    ({ dataStore, ethUsdMarket, ethUsdSpotOnlyMarket, wnt, usdc, wbtc, solUsdMarket, baseOrderUtils } =
      fixture.contracts);
  ...
  it("Swap with same market repeated several times in swap path", async () => {
  ...
  it("Swap with no tokens", async () => {
    ...
    ).to.be.revertedWithCustomError(baseOrderUtils, "EmptyOrder");
  ...
  it("Swap with PI", async () => {
    ...
    expect(await getPoolAmount(dataStore, ethUsdMarket.marketToken, usdc.address)).eq(expandDecimals(99_925, 6));
    expect(await getPoolAmount(dataStore, ethUsdMarket.marketToken, wnt.address)).eq(0);
    ...
    expect(await getPoolAmount(dataStore, ethUsdMarket.marketToken, usdc.address)).eq(expandDecimals(74_925, 6));

    // ~$75 of +PI experienced
    expect(await usdc.balanceOf(user0.address)).eq("25074924999");
    expect(await wnt.balanceOf(user0.address)).eq(0);
  });
});
