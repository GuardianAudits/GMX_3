...
describe("Guardian.Withdrawal", () => {
  ...
  let reader, dataStore, withdrawalHandler, ethUsdMarket, ethUsdSpotOnlyMarket, wnt, usdc, withdrawalUtils;
  let usdcPriceFeed;
  ...
  it("Withdraw market tokens for different market fails", async () => {
    ...
    // Now User0 sneakily tries to use his market tokens for another market (ethUsdSpotOnlyMarket)
    await handleWithdrawal(fixture, {
      create: {
        market: ethUsdSpotOnlyMarket,
        marketTokenAmount: expandDecimals(10 * 1000, 18),
      },
    });
    ...
    // Now withdraw from proper market
    await handleWithdrawal(fixture, {
      create: {
        market: ethUsdMarket,
  ...
  it("User tries to withdraw with 0 market tokens", async () => {
    expect(await wnt.balanceOf(user0.address)).eq(0);
    expect(await usdc.balanceOf(user0.address)).eq(0);
    // User0 attempts withdraw without prior deposits
    await handleWithdrawal(fixture, {
      create: {
        market: ethUsdSpotOnlyMarket,
        marketTokenAmount: expandDecimals(10 * 1000, 18),
    ...
    ).to.be.revertedWithCustomError(withdrawalUtils, "EmptyMarketTokenAmount");
    // No tokens were gained
    expect(await wnt.balanceOf(user0.address)).eq(0);
    expect(await usdc.balanceOf(user0.address)).eq(0);
    expect(await getWithdrawalCount(dataStore)).eq(0);
  ...
  it("Uses price feed tokens", async () => {
    ...
    await handleWithdrawal(fixture, {
      create: {
        market: ethUsdSpotOnlyMarket,
        marketTokenAmount: expandDecimals(50000, 18),
      },
      execute: {
        tokens: [wnt.address],
    ...
    // User0 gets 50% of each token
    expect(await wnt.balanceOf(user0.address)).eq(expandDecimals(5, 18));
    expect(await usdc.balanceOf(user0.address)).eq(expandDecimals(25_000, 6));
  });
});
