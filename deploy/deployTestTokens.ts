...
const func = async ({ getNamedAccounts, deployments, gmx, network }: HardhatRuntimeEnvironment) => {
  ...
  for (const [tokenSymbol, token] of Object.entries(tokens)) {
    ...
    let contract = "MintableToken"
    if (token.wrappedNative) contract = "WNT"
    else if (token.accurateWrappedNative) contract = "WNTAccurate"

    const { address, newlyDeployed } = await deploy(tokenSymbol, {
      from: deployer,
      log: true,
      contract,
      args: token.wrappedNative || token.accurateWrappedNative ? [] : [tokenSymbol, tokenSymbol, token.decimals],
    });
